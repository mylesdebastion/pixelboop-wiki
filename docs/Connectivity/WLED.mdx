...
A single tap opens the Rainbow Menu (the WLED / VJ / lighting settings dialog), where you enter the WLED device IP, test the connection and configure visualizers. A "Rainbow Menu" tooltip confirms it.

### Hold: Toggle WLED output

Holding the button for about half a second toggles WLED output on or off without opening anything. A "WLED On" / "WLED Off" tooltip confirms.
